...
tp <- temp %>%
  group_by(acriss_code) %>%
  summarise(top10avg_price = mean(head(order(price_per_day, decreasing = F ), 10)))

# percentile
tp_percentile <- temp %>%
  group_by(acriss_code) %>%
  summarize(target_price = quantile(order(price_per_day, decreasing=F), probs=0.95))

# 
tp_top5percent <- temp %>%
  group_by(acriss_code) %>%
  summarize(target_price = mean(head(order(price_per_day, decreasing = F), ceiling(n() * 0.05))))
